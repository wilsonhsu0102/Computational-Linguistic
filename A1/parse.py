...
class PartialParse(object):
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        if transition_id == self.shift_id:
            if self.next == len(self.sentence):
                raise ValueError("No more words to shift")
            self.stack.append(self.next)
            self.next += 1
        elif transition_id == self.left_arc_id:
            if len(self.stack) < 2:
                raise ValueError("Not enough words in stack to place arc") 
            if self.stack[-2] == 0:
                raise ValueError("ROOT cannot be a dependent")
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        """
        # ****BEGIN YOUR CODE****
        # Find all dependents, sort, then slice
        deps = [arc[1] for arc in self.arcs if arc[0] == sentence_idx]
        deps.sort()
        deps = deps[:n]
        # ****END YOUR CODE****
        return deps
    ...
    def get_oracle(self, graph: DependencyGraph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        # ****BEGIN YOUR CODE****
        if len(self.stack) < 2: # can only shift
            transition = self.shift_id
        else:
            top_item = self.stack[-1]
            second_item = self.stack[-2]
            # Place left arc if second item is not ROOT and second_item <- top_item  in graph
            if get_head(second_item, graph) == top_item:
                transition, deprel_label = self.left_arc_id, get_deprel(second_item, graph)
            # Place right arc if second_item -> top_item in graph
            elif get_head(top_item, graph) == second_item:
                right_arc = True
                # If there is un-connected right dependent for top_item, do not place right arc
                for dep in get_right_deps(top_item, graph):
                    if dep >= self.next:
                        right_arc = False
                if right_arc:
                    transition, deprel_label = self.right_arc_id, get_deprel(top_item, graph)
                else:
                    transition = self.shift_id
            else:
                transition = self.shift_id
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for i in range(len(td_pairs)):
            pp = batch[i]
            try:
                pp.parse_step(td_pairs[i][0], td_pairs[i][1])
            except(ValueError):
                unfinished_parses.remove(pp)
            if pp.complete:
                unfinished_parses.remove(pp)
    arcs = [p.arcs for p in partial_parses]
    # ****END YOUR CODE****
    return arcs
