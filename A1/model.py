...
class ParserModel(nn.Module):
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # ****BEGIN YOUR CODE****
        word_look_up = self.word_embed(word_id_batch)
        tag_look_up = self.tag_embed(tag_id_batch)
        deprel_look_up = self.deprel_embed(deprel_id_batch)
        x_w = self.reshape_embedded(word_look_up)
        x_t = self.reshape_embedded(tag_look_up)
        x_l = self.reshape_embedded(deprel_look_up)
        x = torch.cat((x_w, x_t, x_l), dim=1)
        # ****END YOUR CODE****
        return x
